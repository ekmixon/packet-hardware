...
class Component(object):
    def __init__(self, lshw=None, element=None):
        self.component_type = f"{self.__class__.__name__}Component"
    ...
    def post(self, tinkerbell):
        if response := utils.http_request(tinkerbell, str(self), "POST"):
    ...
    def post_all(cls, components, tinkerbell):
        ...
        if response := utils.http_request(tinkerbell, components_json, "POST"):
